...
const DEFAULTS = {
  ...
  notFoundHandler: (ctx, next) => ctx.throw(404),
  noMethodHandler: (ctx, next) => DEFAULTS.notFoundHandler(ctx, next)
}
...
async function router (ctx, next) {
  ...
  // - route not found
  if (!route) return asyncCall(config(router).notFoundHandler, router, ctx, emptyNext)
  // - handle middleware
  ...
  if (!(method && method[HANDLER])) return asyncCall(config(router).noMethodHandler, router, ctx, emptyNext)
  ...
  if (ctxRolesFetcher && method[NAME]) {
    let roles = await asyncCall(ctxRolesFetcher, router, ctx, emptyNext)
    if (!RBAC.match(method[NAME], roles)) {
      return prohibitHandler ? asyncCall(prohibitHandler, router, ctx, emptyNext) : ctx.throw(403)
    }
  }
